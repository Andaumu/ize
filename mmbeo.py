    ...
    try:
        ...
        if resp.status_code == 200:
            for item in re.split(r"\s+", resp.text.strip()):
                ...
                proxies.append(item if item.startswith("http://") else f"http://{item}")
    except: pass
    return proxies
...
def check_proxies_batch_http(proxies, test_url, timeout=15, max_workers=200, stop_event=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as exec:
        fut = {exec.submit(check_proxy_http, p, test_url, timeout): p for p in proxies}
...
def get_code_info(code):
    ...
    try:
        r = requests.post('https://web3task.3games.io/v1/task/redcode/detail', headers=headers,
                          json={'code':code,'os_ver':'android','platform':'h5','appname':'app'}, timeout=5).json()
...
async def run_proxy_check(update, context, long_mode=False):
    ...
    if rt.get("proxy_task") and not rt["proxy_task"].done():
        await update.callback_query.edit_message_text("⚠️ Scan proxy đang chạy, vui lòng đợi."); return
    ...
    def block():
        try:
            ...
            else: return check_proxies_batch_http(proxies, "http://ip-api.com/json/", 15, 200, ev)
...
# ==================== SCAN ACCOUNT LIÊN QUÂN (ĐÃ SỬA TIMEOUT) ====================
scan_lq_tasks = {}
def create_garena_account(session):
    """Thử API, tăng timeout và thêm retry"""
    for attempt in range(3):
        try:
            r = session.get("https://keyherlyswar.x10.mx/Apidocs/reg/reglq.php", timeout=120)  # tăng timeout lên 30s
...
async def scan_lq_worker(chat_id, quantity, context):
    ...
    for i in range(1, quantity + 1):
        ...
        # gọi create_garena_account (đã có retry bên trong)
...
# ==================== SPAM SMS ====================
# COPY TOÀN BỘ DANH SÁCH all_otp_funcs TỪ FILE 3.py VÀO ĐÂY
# (bao gồm tất cả các hàm send_otp_via_... và mảng all_otp_funcs)
...
]  # <--- THAY THẾ DÒNG NÀY BẰNG MẢNG 76 HÀM
...
async def button_callback(update, context):
    ...
    if data == "main_menu": await query.edit_message_text("🏠 Menu", reply_markup=main_menu_keyboard())
        ...
        elif act == "add_account": context.user_data["expect_xw"] = "account_link"; await query.edit_message_text("Gửi link tài khoản.")
        ...
        elif act == "set_threshold": context.user_data["expect_xw"] = "threshold"; await query.edit_message_text(f"Ngưỡng hiện tại: {get_user_xworld_data(uid)['threshold']}\nGửi số mới.")
        elif act == "add_code": context.user_data["expect_xw"] = "code"; await query.edit_message_text("Gửi code.")
