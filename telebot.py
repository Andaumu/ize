...
# ==================== STOPBOT / STARTBOT (ĐÃ SỬA is_admin) ====================
...
# ==================== KEY ADMIN COMMANDS (ĐÃ SỬA is_admin) ====================
...
async def cmd_genmasterkey(update, context):
    if not is_owner(update.effective_user.id): await update.message.reply_text("❌ Chỉ chủ bot."); return
    duration = context.args[0] if context.args else None
...
async def menu_command(update, context):
    ...
    commands_text = (
        ...
        "Chỉ chủ bot: /genmasterkey, /setowner"
    )
