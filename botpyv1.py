...
def generate_vietqr(so_tai_khoan, ten_chu_tk, ma_bin, so_tien, noi_dung):
    ...
    ten_encoded = quote(str(ten_chu_tk).strip())
    noi_dung_encoded = quote(str(noi_dung)[:25].strip())
    # Dùng vietqr.io (API chính thức, trả về ảnh PNG, không bị 400)
    return (
        f"https://img.vietqr.io/image/{ma_bin}-{so_tai_khoan}-compact2.png"
        f"?amount={so_tien}"
        f"&addInfo={noi_dung_encoded}"
        f"&accountName={ten_encoded}"
    )
...
async def button_callback(update, context):
    ...
    if data == "main_menu": await query.edit_message_text("🏠 Menu", reply_markup=main_menu_keyboard())
    ...
    elif data == "xw_settings": await query.edit_message_text("⚙️ Cài đặt XWorld", reply_markup=xw_settings_keyboard())
    elif data == "xw_add_account": context.user_data["expect_xw"] = "account_link"; await query.edit_message_text("🔗 Nhập link tài khoản XWorld (chứa ?userId=...&secretKey=...):")
    ...
    elif data == "xw_set_threshold": context.user_data["expect_xw"] = "threshold"; await query.edit_message_text("🎚️ Nhập ngưỡng slot tối thiểu (VD: 5):")
    elif data == "xw_add_code": context.user_data["expect_xw"] = "code"; await query.edit_message_text("➕ Nhập code XWorld cần canh:")
        ...
        if chat_id in stress_tasks: stress_tasks[chat_id].set(); stress_tasks.pop(chat_id, None); await query.edit_message_text("🛑 Đã dừng test.", reply_markup=ddos_menu_keyboard())
        else: await query.edit_message_text("ℹ️ Không có test đang chạy.", reply_markup=ddos_menu_keyboard())
...
# ==================== LỆNH TẮT (ĐÃ LOẠI DUPLICATE) ====================
async def cmd_proxy(update, context):
    if not is_owner(update.effective_user.id) and not check_user_key(update.effective_user.id):
        await request_key(update, context); return
