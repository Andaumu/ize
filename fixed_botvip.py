...
BOT_TOKEN = "8504373990:AAFT6lIh3A-6q2_4eVlsgrj6wDAid8tozIo"
...
SEPAY_ACCOUNT_NUMBER = "05237890382763"  # ← THAY BẰNG STK MB BANK THẬT CỦA BẠN
SEPAY_BIN = "970422"
NAP_RATE = 1
...
def update_user_profile(user_id, update):
    ...
    if "nap_code" not in user_data_store[uid]:
        user_data_store[uid]["nap_code"] = f"NAP{uid}"
    save_all_data()
...
def fetch_proxies_from_url_http(url, table_id=None, table_class=None, proxies=None):
    ...
    proxies_list = []
    try:
        r = requests.get(url, headers=headers, timeout=8, proxies=proxies); r.raise_for_status()
...
def check_proxy_http(proxy, test_url, timeout=8):
    try:
        s = time.time()
...
def check_proxies_batch_http(proxies_list, test_url, timeout=8, max_workers=200, stop_event=None, user_proxies=None):
...
def check_proxies_from_file_http(path, test_url, timeout=8, max_workers=200, stop_event=None, user_proxies=None):
    try:
...
def multi_round_check_http(initial, test_url, rounds=3, interval_min=5, timeout=8, max_workers=200, stop_event=None, user_proxies=None):
...
def xw_settings_keyboard():
 ...
 #==================== NẠP IZE ====================
def generate_vietqr(account_no, account_name, bin_code, amount, content):
    amount_str = str(amount)
    content = content[:25]
    return f"https://vietqr.co/api/generate/{bin_code}/{account_no}/{account_name}/{amount_str}/{content}?style=1&logo=1&isMask=1"
...
async def cmd_nap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    qr_url = generate_vietqr(SEPAY_ACCOUNT_NUMBER, "IZE BOT", SEPAY_BIN, amount, nap_code)  # ✅ Đã sửa SEPAY_BIN
...
async def check_nap_transactions(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        r = requests.get("https://my.sepay.vn/userapi/transactions/list", headers=headers,
                         params={"account_number": SEPAY_ACCOUNT_NUMBER, "limit": 20}, timeout=10)  # ✅ Đã sửa SEPAY_ACCOUNT_NUMBER
...
def upload_to_supabase(local_path, remote_name):
    ...
    try:
        ...
        logger.error(f"Upload lỗi: {e}"); return None
...
async def run_proxy_check(update, context, long_mode=False):
    ...
    if len(live) <= 50: await msg.edit_text(format_proxy_list_short(live), parse_mode="Markdown")
        ...
        if public_url:
            await msg.reply_text(f"📥 Tải file: {public_url}")
        else:
...
async def handle_proxy_file_upload(update, context):
    ...
    if len(live) <= 50: await update.message.reply_text(format_proxy_list_short(live))
        ...
        if public_url:
            await update.message.reply_text(f"📥 Tải file: {public_url}")
        else:
            await update.message.reply_document(open(p,"rb"), filename="proxy_live.txt")
...
async def run_stress_test(update, context, chat_id, url, threads, duration):
    ...
    workers = [threading.Thread(daemon=True, target=ddos_worker, args=(url, ev, q)) for _ in range(threads)]
...
async def cmd_giftize(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        await update.message.reply_text("❌ Chỉ chủ bot mới có quyền này.")
        return
